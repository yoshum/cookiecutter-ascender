...
To create a project from Cookiecutter Ascender, run the following command:

```shell
$ cookiecutter gh:yoshum/cookiecutter-ascender
```

Then, you will be asked to enter several parameters. Here is the list of the parameters
and main places where they are used:

* `project_name`
  * Human-readable project name
  * Used as the title of `README.md`
* `project_slug`
  * Must match the case-insensitive regex: `^([A-Z0-9]|[A-Z0-9][A-Z0-9._-]*[A-Z0-9])$`
  * Used as the `name` parameter in the `[tool.poetry]` section of `pyproject.toml`, the name of the project root directory generated, etc.
* `package_name`
  * Must be a valid package name in Python
* `author`
* `email`
* `description`
* `version`
  * Used to fill the respective fields in the `[tool.poetry]` section of `pyproject.toml`
  * `version` is also set as a value of `__version__` in `src/{{ package_name }}/__init__.py`

Now, the project is ready. For a detailed description of what are in the Ascender template,
see the original README appended below.
